...
def normalize_video_url(url: str) -> str:
    """Normalize site-specific share URLs into direct video page URLs where possible."""
    douyin_modal_match = re.search(r"[?&]modal_id=(\d+)", url)
    if "douyin.com/" in url and douyin_modal_match:
        return f"https://www.douyin.com/video/{douyin_modal_match.group(1)}"
    return url
...
def download_video(
    ...
    if cookies_from_browser:
        ...
        print(f"Using yt-dlp browser cookies from: {cookies_from_browser}", file=sys.stderr)
...
def main():
    ...
    parser.add_argument(
        ...
        help="Browser name for yt-dlp cookies, for example edge or chrome",
    )
    parser.add_argument("--raw", action="store_true", help="Output raw text instead of JSON")
    args = parser.parse_args()
    args.url = normalize_video_url(args.url)
    local_input_path = os.path.abspath(os.path.expanduser(args.url))
    is_local_input = os.path.isfile(local_input_path)
    ...
    # Download for non-YouTube, or use a local video file directly.
    if is_local_input and args.download_only:
        if args.output:
            output_path = os.path.abspath(os.path.expanduser(args.output))
            os.makedirs(os.path.dirname(output_path) or ".", exist_ok=True)
            shutil.copy2(local_input_path, output_path)
            print(f"Video saved to: {output_path}")
        else:
            print(f"Local video file: {local_input_path}")
        return
    ...
    try:
        if is_local_input:
            print(f"Using local video file: {video_path}", file=sys.stderr)
        else:
        ...
        if not is_local_input and not args.keep and not args.download_only and os.path.exists(video_path):
